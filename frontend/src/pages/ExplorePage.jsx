...
const ExplorePage = () => {
  ...
  const exploreRepos = async (language) => {
    ...
    try {
      ...
      //   // headers: {
      //   //   authorization: `token ${import.meta.env.GITHUB_API_KEYS}`,
      //   // },
      // });
      const res = await fetch("/api/explore/repos/" + language, { credentials: "include" });
      console.log("res:", res);
      ...
      <div className="bg-glass max-w-2xl mx-auto rounded-md p-4">
        ...
        {repos.length > 0 && (
          <h2 className="text-lg font-semibold text-center my-4">
            <span className="bg-blue-100 text-blue-800 text-base font-extrabold me-2 px-2.5 py-0.5 rounded-full ">{selectedLanguage.toUpperCase()} </span>
            Repositories
          </h2>
        )}
